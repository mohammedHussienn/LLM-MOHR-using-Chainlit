...
import tempfile
import shutil
from utils.paths import get_config_path, get_file_path
...
class QueryGeneration:
    ...
    def __init__(self, schema_file_path: str, database, llm=None):
        ...
        try:
            with open(str(schema_file_path), 'r', encoding='utf-8') as f:
                self.schema = f.read()
        except UnicodeDecodeError:
            with open(str(schema_file_path), 'r', encoding='latin-1') as f:
                self.schema = f.read()
                
        # Load examples
        examples_path = get_config_path('examples.txt')
        try:
            with open(examples_path, 'r', encoding='utf-8') as f:
                self.examples = f.read()
        except UnicodeDecodeError:
            with open(examples_path, 'r', encoding='latin-1') as f:
                self.examples = f.read()
...
class DatabaseBackend:
    ...
    def __init__(self, schema_file_path: str):
        self.schema_file_path = str(get_config_path(schema_file_path))
    ...
    def save_visualization(self, filename: str, content: Any) -> Path:
